...
class QrController extends Controller
{
     ...
     */
    public function viewqreAjaxList(Request $request)
    {
        ...
        foreach ($groups as $cate) {
            ...
            $data[] = array(
                ...
                $cate->phone,
                $cate->address,
                '<a href="' . route("admin.qr.add", ["id" => $cate->id]) . '" class="btn btn-success btn-sm"><i class="fa fa-plus"></i></a> | <a href="' . route("admin.qr.view", ["id" => $cate->id]) . '" class="btn btn-info btn-sm"  data-id="' . $cate->id . '" data-name="' . $cate->name . '" data-phone="' . $cate->phone . '" data-address="' . $cate->address . '" data-delivery_fee="' . $cate->delivery_fee . '"> <i class="fa fa-eye"></i></a>',
            );
        }
    ...
    public function addqr($id)
    {
        try {
            ...
            return view('admin.qr.add', compact('food', 'id'));
        } catch (\Exception $e) {
    ...
    public function viewqr($id)
    {
        ...
        // dd($branch);
        if (!$branch) {
            return redirect()->route('admin.qr.index')->withErrors(['error' => 'Branch not found.']);
        }
        $price = TableQr::where('branch_id', $id)->get();
        // dd( $price);
        $countproduct = count($price);
        // dd( $countproduct);
        if ($countproduct == 0) {
            return redirect()->route('admin.qr.index')->withErrors(['error' => 'No Qr added in this Branch yet.']);
        }
        // dd( $countproduct);
        return view('admin.qr.view', compact('price', 'id'));
    }
    public function viewqrAjaxList(Request $request)
    {
        // dd($request->id);
        if (isset($_GET['search']['value'])) {
            $search = $_GET['search']['value'];
        ...
        $total = TableQr::leftjoin('branches', 'branches.id', 'table_qr.branch_id')
            ->select('table_qr.*',  'branches.name as breanchNmae')
            ->where('table_qr.branch_id', $request->id)
            ->where(function ($query) use ($search) {
                $query->orWhere('table_qr.table_no', 'like', '%' . $search . '%');
            })->count();

        $groups =  TableQr::leftjoin('branches', 'branches.id', 'table_qr.branch_id')
            ->select('table_qr.*',  'branches.name as breanchName','table_qr.status as tableStatus')
            ->where('table_qr.branch_id', $request->id)
            ->where(function ($query) use ($search) {
                $query->orWhere('table_qr.table_no', 'like', '%' . $search . '%');
            })
        ...
        foreach ($groups as $cate) {
            ...
            $status = '<button class="statusVerifiedClick btn ' . ($cate->tableStatus == 1 ? "btn-success btn-sm" : "btn-danger btn-sm") . '  " data-status="' . ($cate->tableStatus == 1 ? '0' : '1') . '" data-id="' . $cate->id . '">' . ($cate->tableStatus == 1 ? "Active" : "De-Active") . '</button>
                       ';
            $data[] = array(
                $i++,
                $cate->table_no,
                $cate->breanchName,
                $status,
                '<a  href="data:image/png;base64,' . $cate->qr_base64 . '" class="editCategory btn btn-info btn-sm " download> <i class="fa fa-download"></i></a> |
    ...
    public function qrStatusUpdate(Request $request)
    {
        $where = array('id' => $request->id);
        $data = array(
            'status' => $request->status,
        ...
        $update = TableQr::where($where)->update($data);
    ...
    public function qrDelete(Request $request)
    {
        try {
            ...
            $where = array('id' => $request->id);
            // dd($where);
            $del = TableQr::where($where)->delete();
    ...
    //     $brach_id = $request->branch_id;

    //     $branch = Branches::find($brach_id);
    //     $total_tables = TableQr::where('branch_id',$brach_id)->count();

    //     // dd($total_tables);


    //     if($branch->table_no > $total_tables ){

    //         $url  =  url('/login/'.$brach_id.'/'.$table_no.' ');
    //         // dd($url);
    ...
    //         $response = $writer->writeString($url);
    //         $imgPath = base64_encode($response);
    //       $image = '<a href="data:image/png;base64, '.$imgPath.'" download><img src="data:image/png;base64, '.$imgPath.' " /></a>';

    //       $data = new TableQr();

    //       $data->table_no	= $table_no;
    //       $data->branch_id	= $brach_id;
    //       $data->qr_base64	= $imgPath;
    //       $data->url	= $url;
    //       $data->save();
    ...
    public function createqr(Request $request)
{
    ...
    // Check if table number already exists in that branch
    $exists = TableQr::where('branch_id', $brach_id)
    ...
    if ($exists) {
        return response()->json([
            'status' => 'duplicate',
            'message' => 'This table number already has a QR code. Please enter a new table number.'
        ]);
    }
    ...
    // If branch still allows more tables
    if ($branch->table_no > $total_tables) {
        ...
        return response()->json([
            'status' => 'error',
            'message' => 'Table limit exceeded for this branch.'
        ]);
    }
}
