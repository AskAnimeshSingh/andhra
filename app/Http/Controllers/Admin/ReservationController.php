...
class ReservationController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.reservation.index',compact('branch'));
    }

    /**
     * @method use for show coupon ajax list
     */
    public function reservationeAjaxList (Request $request)
    {
        ...
        // dd($nameOrder, $orderType);
        $total = Reservation::orWhere(function ($query) use ($search) {
            $query->orWhere('reservations.fname', 'like', '%' . $search . '%');
            $query->orWhere('reservations.lname', 'like', '%' . $search . '%');
            $query->orWhere('reservations.email', 'like', '%' . $search . '%');
            $query->orWhere('reservations.phone', 'like', '%' . $search . '%');
        ...
        ->when($branchId, function ($query, $branchId) {
            if ($branchId === null) {
                ...
                return $query->where('reservations.branch_id', $branchId); 
            }
        })
        ->count();

        $groups = Reservation::orWhere(function ($query) use ($search) {
            $query->orWhere('reservations.fname', 'like', '%' . $search . '%');
            $query->orWhere('reservations.lname', 'like', '%' . $search . '%');
            $query->orWhere('reservations.email', 'like', '%' . $search . '%');
            $query->orWhere('reservations.phone', 'like', '%' . $search . '%');
        ...
        foreach ($groups as $cate) {
            ...
            $tables = TableQr::where('branch_id', $cate->branch_id)->get();

            $tbhtml = '<select class="classtbl" id="table_select">';
            $tbhtml .= '<option value="">Select a Table</option>'; // Adding a default option
            
            foreach ($tables as $info) {
                // Check if the current table number matches the selected table number ($cate->table_no)
                $selected = ($cate->table_no == $info->table_no) ? 'selected' : '';
            
                $tbhtml .= '<option value="' . $info->table_no . '" resId="' . $cate->id . '" ' . $selected . '>' . $info->table_no . '</option>';
            }
            ...
            $status = '<button class="statusVerifiedClick btn ' . ($cate->status == 1 ? "btn-success btn-sm" : "btn-danger btn-sm") . '  " data-status="' . ($cate->status == 1 ? '0' : '1') . '" data-id="' . $cate->id . '">' . ($cate->status == 1 ? "Active" : "De-Active") . '</button>
                       ';
            $status1 = '<select class="status" name="'.$cate->id .'" id="'.$cate->id .'">
            <option value="1" '.($cate->status == 1 ? 'selected' : '').'>Pending</option>
            <option value="2" '.($cate->status == 2 ? 'selected' : '').'>Accept</option>
            <option value="3" '.($cate->status == 3 ? 'selected' : '').'>Cancel</option>
        </select>';     
            $data[] = array(
                $i++,
    ...
    public function addFoodPrice($id)
    {
        try {
            ...
            return view('admin.reservation.add', compact('food', 'id'));
        } catch (\Exception $e) {
    ...
    public function viewPrice($id)
    {
        ...
        return view('admin.reservation.view', compact('price', 'id'));
    }
    ...
    public function editPrice($id)
    {
        ...
        return view('admin.reservation.edit', compact('getData', 'food','id'));
    }
    ...
    public function restable(Request $request){
        // dd($request->all());
        $data = Reservation::find($request->id);
        ...
        $checktb = Reservation::whereDate('day', $data->day)
        ->where('branch_id', $data->branch_id)
        ->where('table_no', $request->table)
        ->where(function ($query) use ($data) {
            $query->where(function ($query) use ($data) {
                $query->whereBetween('start_from', [$data->start_from, $data->till])
                      ->orWhereBetween('till', [$data->start_from, $data->till]);
            })
            ->orWhere(function ($query) use ($data) {
                $query->where('start_from', '<=', $data->start_from)
                      ->where('till', '>=', $data->till);
            });
        })
        ->get();
       ...
       $count= count($checktb);
        ...
        if($count == 0){
            ...
            $data->table_no = $request->table;
            $data->save();
            return response()->json(['message' => 'Data saved successfully'], 200);
        } else {
            // Respond with an error message or handle the condition as needed
            return response()->json(['error' => 'Table is already booked choose another table'], 400);
        }
