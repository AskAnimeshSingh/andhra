...
class IndGrpController extends Controller
{
    /**
    ...
    {
        return view('admin.ingredient_group.index');
    }

    /**
     * @method use for show coupon ajax list
     */
    public function ind_grp_AjaxList(Request $request)
    {
        ...
        $total = IndGrp::orWhere(function($query) use ($search){
            $query->orWhere('name' , 'like' , '%'. $search.'%');
        })->count();

        $groups = IndGrp::orWhere(function($query) use ($search){
            $query->orWhere('name' , 'like' , '%'. $search.'%');
        })
        ...
        foreach ($groups as $cate) {
            ...
            $data[] = array(
                ...
                $cate->name,
                $status,
                '<a  href="javascript:void(0)" class="editIndGrp btn btn-info btn-sm "  data-id="'.$cate->id.'" data-name="'.$cate->name.'" > <i class="fa fa-edit"></i></a> |
    ...
    public function store(Request $request)
    {
        ...
        if($save)
        {
            return response()->json(['status' => true , 'msg' => "Ingredient group created successfully"]);
            exit;
        }
        else
        {
    ...
    public function update(Request $request)
    {
        ...
        {
            $data = IndGrp::findOrFail($request->edit_ind_group);
        ...
        if($save)
        {
            return response()->json(['status' => true , 'msg' => "Update ingredient group update successfully"]);
            exit;
        }
        else
        {
    ...
    public function statusUpdate(Request $request)
    {
        ...
        $update = IndGrp::where($where)->update($data);
