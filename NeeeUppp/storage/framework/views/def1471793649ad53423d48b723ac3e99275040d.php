...
<section class="section">
  ...
  <div class="modal fade" id="editModal" tabindex="-1" role="dialog" a  ria-labelledby="formModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="formModal">Edit Supplier</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form class="edit_supplier" method="POST">
              ...
              <div class="form-group">
                  <label>Address:</label>
                  <div class="input-group">
                    <input type="address" class="form-control" placeholder="address" name="address" id="edit_address">
                  </div>
              </div>
...
<script>
  ...
  $("body").on("submit", ".store_supplier", function(e) {
    ...
    $.ajax({
      url: "<?php echo e(route('admin.supplier.store')); ?>",
      type: "POST",
      data: fd,
      dataType: 'json',
      processData: false,
  ...
  var tables = $('#supplier').DataTable({
    ...
    "serverSide": true,
    "ajax": {
      url: "<?php echo e(route('admin.supplier.supplier_ajax_list')); ?>",
  ...
  $("body").on("submit", ".edit_supplier", function(e) {
    ...
    fd.append('_token', "<?php echo e(csrf_token()); ?>");
    $.ajax({
      url: "<?php echo e(route('admin.supplier.update')); ?>",
      type: "POST",
      data: fd,
      dataType: 'json',
      processData: false,
  ...
  $("body").on("click", ".statusVerifiedClick", function(e) {
    ...
    $.confirm({
      ...
      buttons: {
        yes: function() {
          $.ajax({
              url: "<?php echo e(route('admin.supplier.status')); ?>",
              type: 'POST',
              data: fd,
              dataType: "JSON",
              contentType: false,
  ...
  $("body").on("click", ".delet_supplier", function(e) {
    ...
    $.confirm({
      ...
      buttons: {
        yes: function() {
          $.ajax({
              url: "<?php echo e(route('admin.supplier.destroy')); ?>",
              type: 'POST',
              data: fd,
              dataType: "JSON",
              contentType: false,
...
<?php $__env->stopSection(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('admin.layout.layouts', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/restaurant-pos-backend/resources/views/admin/supplier/index.blade.php ENDPATH**/ ?>